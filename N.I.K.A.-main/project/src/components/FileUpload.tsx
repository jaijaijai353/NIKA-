...
const FileUpload: React.FC = () => {
  const { setDataset, setDataSummary, setAIInsights, setIsLoading, setRawDataset } = useDataContext();
  ...
  const excelSerialToDate = (serial: number): Date => {
    const epoch = Date.UTC(1899, 11, 30); // Excel epoch (accounts for 1900 leap year bug)
    const ms = serial * 24 * 60 * 60 * 1000;
    return new Date(epoch + ms);
  };

  const isLikelyExcelSerial = (val: number): boolean => {
    // Reasonable range: ~1950-01-01 (17897) to ~2099 (76701). Use a conservative band.
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      // Normalize potential Excel serial date numbers into Date objects
      ...
      // Keep raw for reference; use dataset for cleaned/active
      setRawDataset(dataset);
      setDataset(dataset); // Use setDataset for initial upload, not updateCleanedData
      setDataSummary(summary);
      setAIInsights(insights);
      setDatasetPreview(normalizedData.slice(0, 5)); // Preview first 5 rows
  ...
  const formatPreviewValue = (val: any) => {
    if (val instanceof Date) {
      try {
        return val.toLocaleDateString("en-GB"); // dd/MM/yyyy
      } catch {
                        ...
                        {datasetPreview.map((row, idx) => (
                          <tr key={idx}>
                            {Object.keys(row).map((col) => (
                              <td key={col} className="border-b border-gray-800 px-2 py-1">{formatPreviewValue(row[col])}</td>
                            ))}
                          </tr>
                        ))}
                      </tbody>
